...
class ACO:
    ...
    def solve(self, problem, num_of_solutions,lst_no_ants, lst_alpha, lst_beta, lst_evaporation_rate, lst_fitness_coefficient, iterations):
...
def main():
    ...
    for instance in instance_2_run:
        with open('C:/Users/Shivani G Shetty/Downloads/group_NIC/a280-n279.txt', 'r', encoding='utf-8') as file:
            ...
            fitness_coefficients = [0.5, 1, 1.5]
            iteration_counts = [10, 20]
            
            # Hyperparameter tuning
            best_hv = 0
            best_params = None
            for params in itertools.product(ant_counts, alphas, betas, evaporation_rates, fitness_coefficients, iteration_counts):
                no_ants, alpha, beta, rho, Q, iterations = params
                
            # Solve the problem with the current parameters
            
            aco= ACO()
            nds = aco.solve(problem, num_of_solutions, lst_no_ants=[no_ants],
                    lst_alpha=[alpha],
                    lst_beta=[beta],
                    lst_evaporation_rate=[rho],
                    lst_fitness_coefficient=[Q],
            ...
            print(f"Params: {params}, HV: {hv_value}")
